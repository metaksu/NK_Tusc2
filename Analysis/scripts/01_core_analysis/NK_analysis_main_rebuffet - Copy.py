...
# Rebuffet et al. (2024) Data - UPDATED TO HARMONY-CORRECTED DATASET (for healthy blood NK cohort and reference markers)
# This file contains pre-computed Harmony batch correction with X_pca_harmony and X_umap_harmony embeddings
...
print("    NOTE: Using Harmony-corrected dataset with pre-computed batch correction")
...
## --- Global plotting standardization (constants) ---
# Heatmap defaults
...
def calculate_effect_sizes(group1_values, group2_values):
    """Calculate simple effect size statistics between two arrays.

    Returns a dictionary with keys: 'cohens_d', 'mean_diff', 'group1_mean', 'group2_mean'.
    """
    g1 = np.asarray(group1_values)
    g2 = np.asarray(group2_values)

    # Handle empty inputs defensively
    ...
    # Use unbiased variance when possible
    var1 = float(np.var(g1, ddof=1)) if n1 > 1 else float(np.var(g1))
    var2 = float(np.var(g2, ddof=1)) if n2 > 1 else float(np.var(g2))
    denom = max(n1 + n2 - 2, 1)
...
def draw_standard_heatmap(
    ...
    *,
    ...
    cell_height=0.9,
    label_padding=3.0,
    square=HEATMAP_SQUARE_CELLS,
    show_xticks=True,
    rotation_x=45,
):
    """Centralized helper to render a consistent heatmap and layout.

    Returns: (fig, ax)
    """
    target_df = layout_df if layout_df is not None else matrix_df
    fig_w, fig_h, left_margin = calculate_heatmap_layout(
        target_df, min_width=min_width, min_height=min_height,
        cell_width=cell_width, cell_height=cell_height, label_padding=label_padding,
    )

    if scale_symmetric:
        abs_max = (
            np.nanmax(np.abs(matrix_df.values))
            if not np.all(np.isnan(matrix_df.values))
            else 0.1
        )
    ...
    sns.heatmap(
        ...
        cbar_kws={"label": cbar_label, "shrink": HEATMAP_CBAR_SHRINK} if cbar_label else None,
    ...
    ax.tick_params(axis="x", labelsize=12, rotation=rotation_x)
...
if adata_blood_source is None:
    ...
    # --- HARMONY INTEGRATION: Use pre-processed Harmony-corrected data directly ---
    # The new dataset is already batch-corrected and properly structured
    print("      Using pre-processed Harmony-corrected data directly")
    print("      This dataset contains:")
    print("        - adata.obsm['X_pca_harmony']: Harmony-corrected PCA embeddings")
    print("        - adata.obsm['X_umap_harmony']: UMAP from corrected embeddings")  
    print("        - adata.raw: Original uncorrected gene expression data for biological analysis")
    print("        - adata.X: Log-normalized, scaled data for highly variable genes only")
    
    adata_blood = adata_blood_source.copy()
    print(f"      adata_blood loaded directly from Harmony-corrected file. Shape: {adata_blood.shape}")
    
    # Verify the Harmony embeddings are present
    if 'X_pca_harmony' in adata_blood.obsm:
        print(f"      Harmony PCA embeddings found: {adata_blood.obsm['X_pca_harmony'].shape}")
    else:
        print("      WARNING: X_pca_harmony not found in adata_blood.obsm")
        
    if 'X_umap_harmony' in adata_blood.obsm:
        print(f"      Harmony UMAP embeddings found: {adata_blood.obsm['X_umap_harmony'].shape}")
    else:
        print("      WARNING: X_umap_harmony not found in adata_blood.obsm")
        
    # Verify the raw data is available
    if adata_blood.raw is not None:
        print(f"      Original gene expression data in .raw: {adata_blood.raw.shape}")
    else:
        print("      ERROR: adata_blood.raw is None - this is required for biological analysis")
    
    # Standardize metadata if needed (this should already be done in Harmony-corrected file)
    if REBUFFET_ORIG_SUBTYPE_COL in adata_blood.obs.columns and REBUFFET_SUBTYPE_COL not in adata_blood.obs.columns:
        print(f"      Standardizing subtype column from '{REBUFFET_ORIG_SUBTYPE_COL}' to '{REBUFFET_SUBTYPE_COL}'")
    ...
    elif REBUFFET_SUBTYPE_COL in adata_blood.obs.columns:
        print(f"      Subtype column '{REBUFFET_SUBTYPE_COL}' already standardized")
    else:
        print(f"      ERROR: Neither '{REBUFFET_ORIG_SUBTYPE_COL}' nor '{REBUFFET_SUBTYPE_COL}' found in metadata")
    ...
    if REBUFFET_SUBTYPE_COL in adata_blood.obs.columns:
        ...
        filtered_cells = original_cell_count - adata_blood.n_obs
        if filtered_cells > 0:
            print(f"      Filtered out {filtered_cells} cells with undefined subtypes")
        else:
            print("      All cells have valid subtype annotations")

    # VERIFICATION: Harmony script already applied comprehensive gene filtering (min_cells=5)
    # No additional filtering needed - verified that enhanced QC in Harmony is sufficient
    if adata_blood.raw is not None:
        print(f"      ✓ Harmony pre-filtering verified: {adata_blood.raw.shape[1]:,} high-quality genes available")
        print("      Enhanced QC (MT filtering, doublet detection, gene filtering) completed in Harmony pipeline")
    else:
        print("      ⚠️ No raw data available - check Harmony output")
    
    # COMMENTED OUT: Other preprocessing steps now redundant with Harmony-corrected data
    # - Data transformation (log normalization) ✓ Done in Harmony pipeline
    # - Setting .raw attribute ✓ Done in Harmony pipeline  
    # - HVG selection and scaling ✓ Done in Harmony pipeline
...
if adata_blood is None:
    ...
    print(
        "      ERROR: adata_blood.raw is not set. This is required for biological analyses."
    )
else:
    # --- HARMONY INTEGRATION: Use pre-computed Harmony results ---
    print("      Using pre-computed Harmony-corrected embeddings from batch correction pipeline")
    
    # Verify that Harmony embeddings are available
    if 'X_pca_harmony' not in adata_blood.obsm:
        print("      CRITICAL ERROR: X_pca_harmony not found! Cannot proceed without Harmony-corrected embeddings.")
        print("      Please ensure you are using the correct Harmony-corrected H5AD file.")
    else:
        print(f"      Found Harmony PCA embeddings: {adata_blood.obsm['X_pca_harmony'].shape}")
        
        # Copy the Harmony-corrected PCA to the standard location for compatibility
        adata_blood.obsm['X_pca'] = adata_blood.obsm['X_pca_harmony'].copy()
        print("      Copied X_pca_harmony to X_pca for downstream compatibility")
        
        # Use Harmony-corrected embeddings for neighbor graph calculation
        print("      Computing neighbor graph using Harmony-corrected PCA embeddings...")
        print("      Using standard parameters since batch effects have been corrected by Harmony")
        sc.pp.neighbors(
            adata_blood, 
            use_rep='X_pca_harmony',  # Use Harmony-corrected embeddings directly
            n_neighbors=30,           # Standard parameter allowing biological structure
            random_state=RANDOM_SEED
        )
        print("      Neighbor graph computation complete using Harmony embeddings.")
        
        # Check if Harmony UMAP is available, otherwise compute new UMAP
        if 'X_umap_harmony' in adata_blood.obsm:
            print(f"      Found pre-computed Harmony UMAP: {adata_blood.obsm['X_umap_harmony'].shape}")
            # Copy Harmony UMAP to standard location for compatibility
            adata_blood.obsm['X_umap'] = adata_blood.obsm['X_umap_harmony'].copy()
            print("      Using pre-computed Harmony UMAP for visualization")
        else:
            # Compute UMAP from Harmony-corrected neighbor graph with standard parameters
            print("      Computing UMAP from Harmony-corrected neighbor graph...")
            print("      Using standard parameters to reveal biological NK subtype structure")
            sc.tl.umap(
                adata_blood, 
                random_state=RANDOM_SEED,
                min_dist=0.5,  # Standard parameter for biological structure
                spread=1.0     # Standard parameter for biological structure
            )
            print("      UMAP calculation complete from Harmony-corrected embeddings.")
            
        print(f"      Final UMAP shape: {adata_blood.obsm['X_umap'].shape}")
        
        # Create visualization of Harmony-corrected PCA variance (if available)
        try:
            # Note: We cannot plot variance ratio for pre-computed Harmony PCA
            # Instead, create a simple scatter plot of first 2 Harmony PCs colored by subtype
            fig, ax = plt.subplots(1, 1, figsize=(8, 6))
            
            # Get the first 2 Harmony PCs
            pca_harmony = adata_blood.obsm['X_pca_harmony']
            
            if REBUFFET_SUBTYPE_COL in adata_blood.obs.columns:
                subtypes = adata_blood.obs[REBUFFET_SUBTYPE_COL]
                unique_subtypes = subtypes.cat.categories if hasattr(subtypes, 'cat') else subtypes.unique()
                
                # Create a color map for subtypes
                ...
                ax.scatter(pca_harmony[:, 0], pca_harmony[:, 1], alpha=0.6, s=20)
            
            ax.set_xlabel('Harmony PC1')
            ax.set_ylabel('Harmony PC2')
            ax.set_title('Harmony-Corrected PCA (First 2 Components)')
            plt.tight_layout()
            
            plot_basename_harmony = create_filename(
                "P0_Harmony_PCA_Scatter", context_name="Blood", version="v1"
            )
            save_figure_and_data(
                fig, None, plot_basename_harmony, OUTPUT_SUBDIRS["setup_figs"], None
            )
            print("      Harmony PCA scatter plot saved.")
            
        except Exception as e:
            print(f"      WARNING: Could not create Harmony PCA plot: {e}")
            if 'fig' in locals():
                plt.close(fig)
    
    print("      Harmony-based dimensionality reduction setup complete.")
    print("      IMPORTANT: adata_blood.raw contains original gene expression data for biological analyses.")
    print("      IMPORTANT: adata_blood.X contains log-normalized, scaled data for highly variable genes only.")
...
for context_name, adata_ctx, context_output_base_dir in tang_cohorts_for_crosstab:
    ...
    if "Original_Tang_Subtype" in adata_view_assigned.obs.columns:
        ...
        fig_crosstab, ax_crosstab = draw_standard_heatmap(
            crosstab_norm,
            title=f"Re-annotation Validation: Rebuffet Assignment vs Original Tang Subtypes\n({context_name} - Assigned Cells Only)",
            xlabel="Original Tang Subtype",
            ylabel="Re-annotated Rebuffet Subtype",
            cmap="viridis",
            center=None,
            fmt=".1f",
            annot=True,
            cbar_label="% of Original Tang Subtype",
            scale_symmetric=False,
            rotation_x=45,
            min_width=fig_width,
...
def generate_signature_heatmap(
    ...
    try:
        ...
        fig, ax = draw_standard_heatmap(
            mean_scores_df,
            title=f"{plot_title} in {context_name}",
            xlabel="Assigned NK Subtype",
            ylabel="Signature",
            cmap="icefire",
            center=0,
            fmt=".3f",
            annot=True,
            cbar_label="Mean Signature Score",
            layout_df=mean_scores_df,
            min_width=fig_width,
            min_height=fig_height,
        )
...
def create_blueprint_dotplot(
    ...
    try:
        ...
        dot_plot = sc.pl.dotplot(
            ...
            standard_scale=DOTPLOT_STANDARD_SCALE,
            cmap=DOTPLOT_CMAP,
            dot_max=DOTPLOT_DOT_MAX,
            dot_min=DOTPLOT_DOT_MIN,
...
for context_name, adata_ctx, _ in cohorts_for_characterization:
    ...
    # Enhanced with multiple comparisons analysis and statistical significance visualization
    print(
        f"\n      --- 3.2.2: Proportion of TUSC2 Binary Groups within Subtypes for {context_name} (Enhanced with Statistics) ---"
    )
    try:
        ...
        if crosstab_binary_subtype_prop.empty:
            ...
            print(f"        Performing multiple comparisons analysis for TUSC2+ proportions...")
            ...
            try:
                chi2_stat, chi2_p, chi2_dof, chi2_expected = chi2_contingency(crosstab_binary_subtype)
                print(f"        Overall Chi-square test: χ² = {chi2_stat:.3f}, p = {chi2_p:.4e}, df = {chi2_dof}")
                
                # Extract TUSC2+ proportions for pairwise comparisons
                subtypes = crosstab_binary_subtype.index.tolist()
                tusc2_pos_col = 'TUSC2+' if 'TUSC2+' in crosstab_binary_subtype.columns else crosstab_binary_subtype.columns[1]
                
                # Pairwise Fisher's exact tests for TUSC2+ vs TUSC2- between subtypes
                pairwise_results = []
                for i, (subtype1, subtype2) in enumerate(combinations(subtypes, 2)):
                    # Create 2x2 contingency table for each pair
                    row1 = crosstab_binary_subtype.loc[subtype1].values
                    row2 = crosstab_binary_subtype.loc[subtype2].values
                    
                    if len(row1) >= 2 and len(row2) >= 2:  # Need both TUSC2+ and TUSC2- categories
                        contingency_2x2 = np.array([row1, row2])
                        
                        # Fisher's exact test (more appropriate for small counts)
                        if contingency_2x2.sum() > 0:
                            oddsratio, fisher_p = fisher_exact(contingency_2x2)
                            
                            # Calculate proportions for effect size
                            prop1 = row1[1] / row1.sum() if row1.sum() > 0 else 0  # TUSC2+ proportion
                ...
                if pairwise_results:
                    ...
                    p_values = pairwise_df['Fisher_P_Value'].values
                    ...
                    pairwise_df['Significance_Level'] = pairwise_df['FDR_Adjusted_P'].apply(
                        lambda p: '***' if p < 0.001 else '**' if p < 0.01 else '*' if p < 0.05 else 'ns'
                    )
                    
                    # Save statistical results
                    stats_filename = create_filename(
                        "P3_2_Statistics_TUSC2Binary_by_Subtype_Pairwise",
                        context_name=context_name,
                        version="v2_enhanced",
                        ext="csv"
                    )
                    stats_filepath = os.path.join(layer2_stats_dir, stats_filename)
                    pairwise_df.to_csv(stats_filepath, index=False)
                    print(f"        Pairwise comparison statistics saved to {stats_filename}")
                    
                    # Print significant comparisons
                    sig_comparisons = pairwise_df[pairwise_df['Significant_FDR']]
                    if not sig_comparisons.empty:
                        print(f"        Significant pairwise comparisons (FDR < 0.05):")
                        for _, row in sig_comparisons.iterrows():
                            print(f"          {row['Comparison']}: p_adj = {row['FDR_Adjusted_P']:.4e} {row['Significance_Level']}")
                    else:
                        print(f"        No significant pairwise comparisons found after FDR correction.")
                
            except Exception as stat_e:
                print(f"        Warning: Statistical analysis failed: {stat_e}")
                pairwise_df = pd.DataFrame()  # Empty dataframe if stats fail
            
            # --- ENHANCED VISUALIZATION ---
            # Create publication-quality plot with statistical annotations
            fig_l2_bar_binary, ax_l2_bar_binary = plt.subplots(figsize=(12, 8))
            
            # Focus on TUSC2+ proportions for cleaner visualization
            if tusc2_pos_col in crosstab_binary_subtype_prop.columns:
                tusc2_pos_props = crosstab_binary_subtype_prop[tusc2_pos_col]
                tusc2_neg_props = 100 - tusc2_pos_props  # Calculate TUSC2- proportions
                ...
                # Plot bars
                bars_neg = ax_l2_bar_binary.bar(
                    x_positions, tusc2_neg_props, width, 
                    label='TUSC2-', color=TUSC2_BINARY_GROUP_COLORS.get('TUSC2-', '#1f77b4'),
                    alpha=0.8
                )
                bars_pos = ax_l2_bar_binary.bar(
                    x_positions, tusc2_pos_props, width, bottom=tusc2_neg_props,
                    label='TUSC2+', color=TUSC2_BINARY_GROUP_COLORS.get('TUSC2+', '#d62728'),
                    alpha=0.8
                )
                
                # Add statistical significance brackets if we have results
                if 'pairwise_df' in locals() and not pairwise_df.empty:
                    sig_comparisons = pairwise_df[pairwise_df['Significant_FDR']]
                    
                    if not sig_comparisons.empty:
                        # Add significance brackets for top significant comparisons (max 3 to avoid clutter)
                        top_sig = sig_comparisons.nsmallest(3, 'FDR_Adjusted_P')
                        
                        bracket_height = 105  # Start above the 100% line
                        bracket_increment = 8
                        
                        for idx, (_, row) in enumerate(top_sig.iterrows()):
                            subtype1, subtype2 = row['Subtype_1'], row['Subtype_2']
                            sig_level = row['Significance_Level']
                            
                            # Get x-positions for the subtypes
                            try:
                                x1 = subtypes.index(subtype1)
                                x2 = subtypes.index(subtype2)
                                
                                # Draw significance bracket
                                y_bracket = bracket_height + (idx * bracket_increment)
                                ax_l2_bar_binary.plot([x1, x1, x2, x2], 
                                                     [y_bracket-2, y_bracket, y_bracket, y_bracket-2], 
                                                     'k-', linewidth=1.5)
                                
                                # Add significance annotation
                                x_center = (x1 + x2) / 2
                                ax_l2_bar_binary.text(x_center, y_bracket + 1, sig_level, 
                                                     ha='center', va='bottom', fontsize=12, fontweight='bold')
                            except ValueError:
                                continue  # Skip if subtype not in list
                ...
                ax_l2_bar_binary.set_xlabel(subtype_col, fontsize=14, fontweight='bold')
                ax_l2_bar_binary.set_ylabel("Proportion of Cells (%)", fontsize=14, fontweight='bold')
                
                # Set x-ticks and labels
                ax_l2_bar_binary.set_xticks(x_positions)
                ax_l2_bar_binary.set_xticklabels(subtypes, rotation=45, ha="right", fontsize=12)
                
                # Add percentage labels on bars
                for i, (neg_prop, pos_prop) in enumerate(zip(tusc2_neg_props, tusc2_pos_props)):
                    if pos_prop > 5:  # Only show label if bar is big enough
                        ax_l2_bar_binary.text(i, neg_prop + pos_prop/2, f'{pos_prop:.1f}%', 
                                            ha='center', va='center', fontweight='bold', fontsize=10)
                
                # Legend and grid
                ax_l2_bar_binary.legend(title=TUSC2_BINARY_GROUP_COL, 
                                       bbox_to_anchor=(1.02, 1), loc="upper left", fontsize=12)
                ax_l2_bar_binary.grid(axis='y', alpha=0.3, linestyle='--')
                ax_l2_bar_binary.set_ylim(0, max(120, bracket_height + 15 if 'pairwise_df' in locals() and not pairwise_df.empty else 105))
                
                # Add sample size annotations
                for i, subtype in enumerate(subtypes):
                    n_cells = crosstab_binary_subtype.loc[subtype].sum()
                    ax_l2_bar_binary.text(i, -8, f'n={n_cells}', ha='center', va='top', fontsize=10, style='italic')
            ...
            print(
                f"        Enhanced bar plot with statistical analysis for {context_name} saved."
            )
    except Exception as e:
        print(
...
for context_name, adata_ctx, _ in cohorts_for_characterization:
    ...
    try:
        ...
        # Dynamic axis limits
        try:
            x_vals = volcano_df["logfoldchanges"].replace([np.inf, -np.inf], np.nan).dropna()
            y_vals = volcano_df["-log10_pvals_adj"].replace([np.inf, -np.inf], np.nan).dropna()
            if len(x_vals) > 0:
                x_abs_max = np.nanquantile(np.abs(x_vals), 0.995)
                x_lim = max(0.5, float(x_abs_max) * 1.1)
            else:
                x_lim = 0.5
            if len(y_vals) > 0:
                y_max = np.nanquantile(y_vals, 0.995)
                y_lim_top = max(5.0, float(y_max) * 1.1)
            else:
                y_lim_top = 10.0
...
if "ref_rebuffet_markers" not in locals() or not ref_rebuffet_markers:
    ...
    def analyze_and_plot_overlap(
        ...
        if not pivot_heatmap.empty:
            ...
            fig, ax = draw_standard_heatmap(
                pivot_heatmap,
                title=f"Top 10 TUSC2 {direction}-regulated Overlapping DEGs in {context}\nvs. Blood Reference Subtype Markers",
                xlabel="Reference Blood NK Subtype",
                ylabel=f"Top Overlapping DEGs ({direction}-regulated)",
                cmap="cividis",
                center=None,
                fmt=".0f",
                annot=False,
                ...
                rotation_x=45,
                min_width=new_width,
                min_height=max(5, pivot_heatmap.shape[0] * 0.4),
                square=False,
            )
...
def perform_go_pathway_score_comparison(
    ...
    try:
        # For each GO database, get gene sets and score pathways using AUCell
        for db in go_databases:
            ...
            try:
                ...
                for pathway_name, pathway_genes in gene_sets.items():
                    ...
                    # Calculate pathway scores using AUCell (robust scoring)
                    try:
                        score_genes_aucell(
                            adata_ctx,
                            available_genes,
                        ...
                        if len(tusc2_pos_scores) >= 3 and len(tusc2_neg_scores) >= 3:
                            ...
                            # Use Hedge's g for small sample correction when needed
                            n1 = len(tusc2_pos_scores)
                            n2 = len(tusc2_neg_scores)
                            pooled_std = np.sqrt(((n1 - 1) * tusc2_pos_scores.var(ddof=1) + 
                                                (n2 - 1) * tusc2_neg_scores.var(ddof=1)) / max(n1 + n2 - 2, 1))
                            d = (tusc2_pos_scores.mean() - tusc2_neg_scores.mean()) / pooled_std if pooled_std > 0 else 0
                            J = 1 - (3 / (4 * (n1 + n2) - 9)) if (n1 + n2) > 2 else 1
                            cohens_d = d * J
...
def perform_go_enrichment_analysis(
    ...
    # Standard GO enrichment analysis - no predefined term filtering
    # Will show top 15 most significant GO terms from unbiased enrichment analysis
    ...
    try:
        ...
        if len(up_genes) >= 5:  # Minimum for meaningful enrichment
            ...
            if up_enrichment.results is not None and not up_enrichment.results.empty:
                ...
                if not up_results.empty:
                    # Sort by significance and take top 15 for standard analysis
                    up_results_sorted = up_results.sort_values('Adjusted P-value').head(15)
                    enrichment_results['upregulated'] = up_results_sorted
                    summary_stats['enriched_pathways'] += len(up_results_sorted)
                    print(f"      Found {len(up_results)} total enriched pathways, showing top 15 for upregulated genes")
        ...
        if len(down_genes) >= 5:  # Minimum for meaningful enrichment
            ...
            if down_enrichment.results is not None and not down_enrichment.results.empty:
                ...
                if not down_results.empty:
                    # Sort by significance and take top 15 for standard analysis
                    down_results_sorted = down_results.sort_values('Adjusted P-value').head(15)
                    enrichment_results['downregulated'] = down_results_sorted
                    summary_stats['enriched_pathways'] += len(down_results_sorted)
                    print(f"      Found {len(down_results)} total enriched pathways, showing top 15 for downregulated genes")
        ...
        # Create comprehensive visualization of top 15 GO terms
        if enrichment_results:
            create_go_enrichment_plot(enrichment_results, context_name, output_dirs, top_n_per_db=15)
...
# REMOVED: def perform_curated_go_enrichment - replaced with standard top 15 GO terms approach
def _REMOVED_perform_curated_go_enrichment(up_genes, down_genes, curated_go_terms, context_name, output_dirs, fdr_threshold):
    """
    ...
    try:
        ...
        if focused_results:
            ...
            if all_focused:
                ...
                # Note: Curated GO visualization removed in favor of standard top 15 approach
...
# REMOVED: def create_curated_go_plot - replaced with standard top 15 GO terms approach  
def _REMOVED_create_curated_go_plot(focused_df, context_name, output_dirs):
...
def create_go_enrichment_plot(enrichment_results, context_name, output_dirs, top_n_per_db=15):
    """Create standard GO enrichment plot showing top 15 most significant terms.
    
    This implementation follows best practices for GO analysis visualization:
    - Shows top 15 most significant terms per direction (up/down regulated) 
    - Uses dot plot with -log10(FDR) on x-axis
    - Point size represents gene count in enriched set
    - Color distinguishes up vs down regulated gene sets
    """

    # Collect and enrich plotting data
    plot_rows = []
    for direction, df in enrichment_results.items():
        if df is None or df.empty:
            continue

        # Ensure necessary fields exist
        df_local = df.copy()
        if 'Gene_set' not in df_local.columns and 'Database' in df_local.columns:
            df_local['Gene_set'] = df_local['Database']

        # Parse GO ID and compute Gene Ratio from Overlap k/M when available
        go_ids = []
        gene_ratios = []
        terms_short = []
        for _, row in df_local.iterrows():
            ...
            go_id = m.group(0)[1:-1] if m else None
            go_ids.append(go_id)

            # Shorten term for readability
            terms_short.append(term[:60] + '...' if len(term) > 60 else term)

            # Gene ratio from Overlap (e.g., '8/120')
            overlap = row.get('Overlap', None)
            if isinstance(overlap, str) and '/' in overlap:
                try:
                    k, M = overlap.split('/')
                    gr = float(k) / float(M) if float(M) > 0 else 0.0
                except Exception:
                    gr = np.nan
            else:
                # Fallback: compute from Genes count if term size unknown
                genes_str = row.get('Genes', '')
                k = len(genes_str.split(';')) if isinstance(genes_str, str) and genes_str else np.nan
                gr = np.nan if np.isnan(k) else float(k)
            gene_ratios.append(gr)

        df_local['GO_ID'] = go_ids
        df_local['Term_Short'] = terms_short
        df_local['Gene_Ratio'] = gene_ratios
        df_local['Neg_Log10_FDR'] = -np.log10(df_local['Adjusted P-value'] + 1e-10)
        df_local['Direction_Label'] = 'TUSC2+ Upregulated' if direction == 'upregulated' else 'TUSC2+ Downregulated'
        df_local['Database'] = df_local['Gene_set'].str.replace('_2023', '').str.replace('GO_', '')

        # Take top n per database by FDR
        for db, sub in df_local.groupby('Database'):
            sub_sorted = sub.sort_values('Adjusted P-value').head(top_n_per_db)
            for _, r in sub_sorted.iterrows():
                plot_rows.append({
                    'Label': f"{r['Database']}: {r['Term_Short']}",
                    'Database': r['Database'],
                    'Neg_Log10_FDR': r['Neg_Log10_FDR'],
    ...
    # Normalize sizes for better readability if Gene_Ratio is raw counts
    size_values = plot_df['Gene_Ratio'].fillna(0.0).values
    if np.nanmax(size_values) <= 1.0:
        size_scaled = np.clip(size_values, 0, 1) * 600 + 50
    else:
        # Assume raw counts; scale relative
        size_scaled = (size_values / (np.nanmax(size_values) + 1e-9)) * 600 + 50
    plot_df['Size'] = size_scaled

    # Create scatter bubble plot
    fig, ax = plt.subplots(figsize=(14, max(8, len(plot_df) * 0.4)))
    markers = {'TUSC2+ Upregulated': 'o', 'TUSC2+ Downregulated': 's'}
    databases = sorted(plot_df['Database'].unique())
    palette = dict(zip(databases, sns.color_palette('Set2', n_colors=len(databases))))
    ...
    for idx, row in plot_df.iterrows():
        ax.scatter(
            ...
            idx,
            s=row['Size'],
            c=[palette[row['Database']]],
            alpha=0.75,
            edgecolors='black',
            linewidth=0.5,
            marker=markers[row['Direction']],
        )

    ax.set_yticks(y_positions)
    ax.set_yticklabels(plot_df['Label'], fontsize=8)
    ax.set_xlabel('-log10(FDR)', fontsize=12)
    ax.set_title(f'Top 15 GO Biological Processes Enriched in TUSC2+ vs TUSC2- NK Cells\n{context_name}', fontsize=14, fontweight='bold')
    ax.grid(axis='x', alpha=0.3)
    ax.axvline(-np.log10(0.05), color='red', linestyle='--', alpha=0.5, linewidth=1)

    # Legends
    from matplotlib.lines import Line2D
    db_handles = [Line2D([0], [0], marker='o', color='w', label=db,
                         markerfacecolor=palette[db], markersize=8, markeredgecolor='black') for db in databases]
    dir_handles = [Line2D([0], [0], marker=m, color='black', label=lab, linestyle='None') for lab, m in markers.items()]
    first_legend = ax.legend(handles=db_handles, title='GO Category', bbox_to_anchor=(1.02, 1), loc='upper left')
    ax.add_artist(first_legend)
    ax.legend(handles=dir_handles, title='Direction', bbox_to_anchor=(1.02, 0.7), loc='upper left')

    plt.tight_layout()

    # Save figure
    plot_filename = f"P3_6_GO_Enrichment_Top15_Standard_{context_name}.png"
    filepath = os.path.join(output_dirs['figures'], plot_filename)
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close()

    print(f"      Standard GO enrichment plot (top 15 terms) saved to {plot_filename}")


print("\nPerforming standard GO enrichment analysis using TUSC2 DEGs...")
print("This shows the top 15 most significantly enriched GO biological processes.")
...
if all_tusc2_impact_stats:
    ...
    def create_category_heatmap(
        ...
        # Enforce biologically grouped y-axis order for Core Functional Capacity
        if basename_suffix == "CoreFunctionalCapacity":
            desired_order = [
        ...
        sns.heatmap(
            ...
            linewidths=HEATMAP_LINEWIDTHS,
            cbar_kws={"label": "Mean Score Difference\n(TUSC2+ vs TUSC2-)", "shrink": HEATMAP_CBAR_SHRINK},
            annot_kws={"size": annot_size, "weight": HEATMAP_ANNOT_WEIGHT},
            square=HEATMAP_SQUARE_CELLS,
            ax=ax,
        )
        ax.set_title(title, fontsize=16, fontweight='bold', pad=20)
    ...
    create_category_heatmap(
        ...
        core_functional_signatures,
        "TUSC2+ Cells Associated Core Functional Capacities",
    ...
    create_category_heatmap(
        combined_tusc2_stats,
        il_signatures,
        "TUSC2+ Cells Associated Interleukin Pathways",
        "InterleukinPathways",
        "Interleukin",
    )
    ...
    create_category_heatmap(
        combined_tusc2_stats,
        neuro_signatures,
        "TUSC2+ Cells Associated Neurotransmitter Receptors",
